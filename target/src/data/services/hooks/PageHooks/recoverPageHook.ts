...
export const useRecoverPage = () => {
  ...
  async function recover(password, password2, email, token) {
    if (
      ...
      setError("");
      try {
        await serviceApi.put("auth/reset-password", {
          email,
          password,
          token,
        }).then((res)=>{
        ...
        setError("Não foi possível alterar sua senha, por favor, tente novamente mais tarde."); //COLOCAR ERRO DEPOIS DE CONFIGURAR BACK
        setLoading(false);
      }
    }
  }
